...
class _FriendsModalState extends State<FriendsModal> {
  ...
  Future<void> _searchFriends(String query) async {
    ...
    try {
      final response = await ApiService.searchFriends(query.trim());
    ...
    try {
      ...
      if (response['status'] == 'success') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Friend request sent to ${friend['display_name'] ?? 'User'}!'),
  ...
  Widget _buildResultsList(ColorScheme colorScheme, TextTheme textTheme) {
    if (!_hasSearched) {
      return Center(
        child: Text(
          'Search for your friends by name or email',
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final userId = friend['id'] as int;
        final name =
            friend['display_name'] ?? friend['name'] ?? 'User #$userId';
...
//           );
//
//           // ✅ Auto-close after 3 seconds — remove the 200ms pop!
...
//
//       // ✅ HACK: Force rebuild by temporarily clearing query
//       final originalQuery = query;
//       query = ''; // Triggers rebuild
//       await Future.delayed(Duration.zero); // Let framework process
//       query = originalQuery; // Restore
//
//       _isLoading = false;
//       _hasSearched = true;
//
//       // ✅ Now force final rebuild
//       showResults(context);
//     } catch (e) {
